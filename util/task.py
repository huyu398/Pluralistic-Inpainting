...
def center_large_mask(img):
    mask = torch.ones_like(img)
    size = img.size()
    x = int(size[1] / 8)
    y = int(size[2] / 8)
    range_x = int(size[1] * 7 / 8)
...
def corner_mask(img):
    ...
    range_y = int(size[2] / 2)
    mask[:, x:range_x, y:range_y] = 0

    return mask

def corner_small_mask(img):
    mask = torch.ones_like(img)
    size = img.size()
    x = 0
    y = 0
    range_x = int(size[1] / 4)
...
def between_center_and_corner_small_mask(img):
    mask = torch.ones_like(img)
    size = img.size()
    x = int(size[1] / 8)
    y = int(size[2] / 8)
    range_x = int(size[1] * 3 / 8)
    range_y = int(size[2] * 3 / 8)
    mask[:, x:range_x, y:range_y] = 0
